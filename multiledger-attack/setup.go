...
// setupSwapClient builds a SwapClient for the attack scenario. It is similar
// to multiledger-channel's setupPaymentClient but always enables the relaxed
// proposal/update handlers (acceptAll=true) and never installs a coordinator.
func setupSwapClient(
	...
	c, err := client.SetupSwapClient(
		...
		chains,
		waddress,
		nil,         // no coordinator notifier in this PoC
		coordinator, // optional trusted coordinator (zero-value disables it)
		true,        // attack/defended flows need the relaxed handlers
		autoWatch,   // attack relies on cross-chain divergence; auto-watch replication would mask v1 onto chain A
	)
	if err != nil {
		panic(err)
	}
