...
# Charlie is the cross-chain coordinator (multiledger-virtual-ckb-eth-coordinated):
# he funds his own on-chain coordinate transactions, so he needs CKB capacity.
FUND_AMOUNT_CHARLIE=${FUND_AMOUNT_CHARLIE:-50000}
...
# Charlie (coordinator) — present only when get_address emitted charlie.txt.
charlie=""
[ -f "$ACCOUNTS_DIR/charlie.txt" ] && charlie=$(extract_address "$ACCOUNTS_DIR/charlie.txt")
...
sleep 5
if [ -n "$charlie" ]; then
  fund_address "$charlie" "$FUND_AMOUNT_CHARLIE"
  sleep 5
fi
