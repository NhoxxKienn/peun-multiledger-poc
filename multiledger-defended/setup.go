...
// setupSwapClient builds a SwapClient for the defended scenario. It is similar
// to multiledger-channel's setupPaymentClient but always enables the relaxed
// proposal/update handlers (acceptAll=true) and accepts the cross-chain
// coordinator wiring (notifier + on-chain coordinator address).
func setupSwapClient(
	bus wire.Bus,
	privateKey string,
	chains [2]client.ChainConfig,
	waddress wire.Address,
	notifier *p2p.RelayCoordinatorNotifier,
	...
	c, err := client.SetupSwapClient(
		...
		chains,
		waddress,
		notifier,    // forward coordination events to the cross-chain coordinator
		coordinator, // wire the coordinator address into the channel params
		true,        // accept the non-final v1 update and asymmetric funding
		false,       // start watchers manually after subscribing — mirrors TestMultiLedgerAttackCoordinate
	)
	if err != nil {
		panic(err)
	}
