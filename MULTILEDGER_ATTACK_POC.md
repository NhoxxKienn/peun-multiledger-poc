...
**The implemented demos do not call `evm_increaseTime`.** Both the Hardhat ETH
devnets and the CKB devnet auto-mine (`mining.interval ≈ 500 ms`), so every
dispute timer — `block.timestamp`-based on both backends — advances in real
wall-clock time during the run. Each demo therefore simply waits the
`challengeDuration` (plus a small margin) after a register, on every ledger
uniformly. Keep `challengeDuration` small (≤ 15 s) so the demo finishes
quickly.

```go
// waitChallenge waits out a dispute window in wall-clock time. Both ledgers
// advance their timers from real block production (CKB blocks; the ETH devnet
// mines every 500 ms), so there is no manual evm_increaseTime.
func waitChallenge(seconds uint64) {
    time.Sleep(time.Duration(seconds+2) * time.Second)
}
```

> Historical note: an earlier draft fast-forwarded the ETH chain with
> `evm_increaseTime` + `evm_mine`. That was dropped because the auto-mining
> devnets make it unnecessary, and waiting wall-clock on both ledgers keeps the
> attack timeline faithful (the dispute windows really do elapse in real time).
> CKB has no `evm_increaseTime` analog regardless — its timeouts only progress
> as the node produces blocks.
...
### 11.7 Why the attack is impossible once the parents are coordinated

The divergent-settlement attack only exists on **un-coordinated** parents. Once
a virtual-locking parent carries a coordinator, the attack is prevented **by
construction**, not merely by timing or by an honest watchtower:

- **The adjudicator forces the virtual sub-state into coordination.** A
  `coordinate` of a parent that locks a virtual channel is rejected unless the
  call carries that virtual sub-state (ETH reverts `"subChannels too short"`;
  PCTS rejects an empty `subChannels`), **and** unless the coordinator's
  signature set covers the parent *and* each sub-state (`coordSigs` length must
  equal `1 + #subChannels`). So there is no realizable "coordinate the parent
  only, leave the virtual to diverge" move — coordinating the parent *is*
  coordinating its virtual.
- **A coordinated channel cannot be force-closed.** Multi-ledger `conclude`
  requires the COORDINATED phase, so the attacker cannot bypass the coordinator
  with a divergent force-close.
- **The watchtower alone is not enough.** The intermediary (Ingrid) only ever
  co-signs the virtual's funding sub-state (vc0); she never holds the later vc1,
  so her watcher cannot refute Bob's stale registration (its refutation is
  rejected `"subChannels too short"`). The watchtower is also liveness-bounded —
  it must detect and refute within each chain's challenge window, which a large
  cross-chain finality gap could defeat. **Recursive coordination has no such
  timing assumption:** the coordinator co-signs one canonical virtual sub-state
  for every ledger, so both settle uniformly regardless of window skew.

This is exactly why the coordinated demos
(`multiledger-virtual-coordinated`, `virtual-ckb-eth-coordinated`) carry **no
attack mode**: the only thing left to demonstrate over coordinated parents is
the honest/defended path, where the recursive `CoordinateVC` pins one global
canonical virtual version on both ledgers. The divergent attack itself is shown
only on the un-coordinated modules (`multiledger-virtual-optimistic`,
...
- **Dispute timers advance in real wall-clock time on both ledgers.** CKB has
  no `evm_increaseTime` analog, and the demos deliberately don't use it on ETH
  either — both devnets auto-mine (~500 ms), so the `block.timestamp` timers
  progress naturally. Every demo waits the full `challengeDuration` after a
  register; keep that constant small (≤ 15 s) or the demo runs long. The
  coordinator's `awaitFinalisationAndCoordinate` also uses wall-clock, so this
...
**Invariant I1 → I1′ (global canonical state extends through the virtual
layer).** The sub-channel PoC's invariant *I1* — "a coordinated channel settles
at one canonical version on every ledger" — is strengthened to *I1′*: the
canonical version is global across the **whole channel tree**, parent *and*
locked virtual sub-channel. Because coordinating a VC-locking parent forces its
virtual sub-state into the same `coordinate` (see §11.7), the recursive
`CoordinateVC` pins one virtual version on every ledger, so the virtual channel —
not just each parent — settles uniformly. I1′ is what the optimistic and
un-coordinated virtual demos violate (vc0 on one ledger, vc1 on the other) and
what the coordinated demos restore.
